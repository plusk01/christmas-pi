...
import time
# from pyalsa.alsamemdebug import debuginit, debug, debugdone
...
for note in notes:
    ...
    event.queue = queue
    event.time += ppq
    event.set_data({'note.note' : note, 'note.velocity' : 64, 'note.duration' : ppq , 'note.off_velocity' : 64, 'note.channel': 9})
